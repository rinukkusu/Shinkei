...
{
	class CustomAuthenticationLoginCommand : ICommandExecutor
	{
		...
		internal CustomAuthenticationLoginCommand(CustomAuthenticationInterface CustomAuthenticationInterface)
		{
		...
		public bool Execute(Command Command, EntUser Executor, CommandMessage Data)
		{
			...
			if (RegisteredUser.HashedPassword.Equals(HashedPassword))
			{
				if (!this.CustomAuthenticationInterface.LoggedInUsers.ContainsKey(Executor.Username))
				{
					...
					Data.SendResponse($"You have been identified as {Nick}.");
				}
				else
				{
					Data.SendResponse($"You are already logged in.");
				}
			}
			else
			{
